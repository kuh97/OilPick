...
**폴백 처리** — 앱 실행 성공 여부를 웹에서 신뢰성 있게 감지할 방법이 없어, **미설치 시 "무동작"** 을 기본값으로 삼는다. 대신 모바일 화면에는 내비 버튼 아래에 _"지도 앱은 설치되어 있어야 열 수 있습니다"_ 를 상시 노출한다. 스토어로 자동 이동시키지 않는다.

| 환경 | 처리 |
| --- | --- |
| 카카오·네이버·티맵 (모바일·인앱) | 각각 `kakaomap://` · `nmap://` · `tmap://` 스킴을 `location.assign`. 앱이 없으면 브라우저에 따라 무동작 또는 오류 페이지 |
| 데스크톱 | 경유지를 포함한 웹 지도 링크를 새 탭으로 (티맵은 웹 길찾기 없음 → 버튼 숨김) |

**공식 참고 문서**

- [카카오맵 URL Scheme](https://apis.map.kakao.com/ios_v2/docs/getting-started/urlscheme/) — 앱 스킴·`vp` 경유지
- [카카오 지도 웹 바로가기](https://apis.map.kakao.com/web/guide/) — 웹 길찾기 URL
- [네이버 앱 URL Scheme](https://developers.naver.com/docs/utils/mobileapp/) — 앱 스킴 규격
- [티맵 앱 실행·다운로드 안내](https://www.tmap.co.kr/tmap2/mobile/run.jsp) — 공식 설치 안내 페이지. 경로 보존 HTTPS 중계 규격은 확인되지 않음
