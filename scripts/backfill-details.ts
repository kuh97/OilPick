/**
 * 시설정보 백필 수동 실행 — docs/MIGRATION-DB.md §7 Phase E.
 *
 ...
 *
 * 필요 환경변수: OPINET_CERT_KEY, DATABASE_URL
 */
...
async function main() {
  ...
  if (limitArg !== undefined && (!Number.isFinite(limitArg) || limitArg <= 0)) {
    throw new Error(`limit 인자가 올바르지 않습니다: ${process.argv[2]}`);
  }
  ...
  if (r.remaining > 0) {
    ...
    info(`남은 대상 ${r.remaining}건 — 하루 ${r.limit}건 기준 약 ${days}일`);
  } else {
    ok("남은 대상 없음 — 백필 완료");
  }
}
