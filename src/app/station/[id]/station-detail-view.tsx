 ...
 *
 * 검색 컨텍스트(스토어의 result)에서 후보를 찾아 보여준다. 외부 내비게이션
 * 링크와 앱 미설치 안내는 ARCHITECTURE.md §5.5를 따른다.
 */
...
const NAVI_APPS: { app: NaviApp; label: string; appName?: string }[] = [
  ...
  { app: "NAVER", label: "네이버지도", appName: "https://oilpick.vercel.app" },
  { app: "TMAP", label: "티맵" },
];
...
export function StationDetailView({ id }: { id: string }) {
  ...
  function handleNaviClick(app: NaviApp, appName?: string) {
    ...
    const appUrl = buildDeeplink(deeplinkInput);

    // 모바일 웹은 앱 설치 여부를 확정할 수 없어 스토어 폴백 없이 스킴만 시도한다.
  ...
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col gap-4 px-4 py-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        {isMobile && (
          <div className="break-keep flex flex-col gap-1 text-xs text-muted-foreground">
            <p>지도 앱은 설치되어 있어야 열 수 있습니다.</p>
            <p>
